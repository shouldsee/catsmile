...
def main():
    ...
    print('[TRYING]')
    def idx2word(i): return conf.dataset.idx2char(i)
    # english_vocab_reversed[i]
    ...
    def callback(outer,inner, out='token'):
        (zi,x,y,z,fs) = outer
        lptok = fs
        # (i,sel,xz,xs) = xc
        for i in range(x.shape[1]):
            ...
            # print(sel.shape)
            # print((sel[0,:,:10]*100).long())
            v = idx2word(conf.model.vocab((z[0:1,i:i+1])).argmax(-1)[0])
            v2= idx2word(lptok[0,i:i+1].argmax(-1)[0])
            v3= idx2word((int(x[0:1,i:i+1])))
            print(wlen(v,7), end=' : ')
            print(wlen(v2,7),end=' : ')
    ...
    for i in range(2):
        ...
        for item in (conf.dataloader):
            out = model.loss(item['idx'],item['x'],None,item['z'])
            # out = model.get_tokens(item['idx'],item['x'], None,it?em['x'])
