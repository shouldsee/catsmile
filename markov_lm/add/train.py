...
def init_conf(CUDA,shuffle, AddModelWithAttention=AddModelWithAttention,ADD_MONITOR_HOOK=1):
    ...
    if 1:
        ...
        conf.lconf = LayerConfig(
            ...
            mask_token_idx = dataset.mask_token_idx,
            graph_dim      = dataset.graph_dim,
            max_position_length = conf.max_position_length,
        ...
        # conf.learning_rate = 0.0001
        # conf.batch_size = 120
        # conf.sample_method = 'simple'
        # conf.sample_method = 'shuffle'
        conf.sample_method = 'mixed'
        # conf.sample_method = 'mcmc'
        # conf.sample_method = 'nll'
        ...
        conf.contrast      =  4
        conf._session_name += f'MAXL{conf.max_len}-'
        conf._session_name += f'TS{conf.thin_sep}-'
        conf._session_name += f'NM{conf.n_mask}-'
        conf._session_name += f'Mix{conf.lconf.mixture_count}-'
        conf._session_name += f'maxPos{conf.max_position_length}-'
        conf._session_name += f'SSK{conf.lconf.step_size_keep}-'
        conf._session_name += f'CK{conf.contrast}-'

        # conf._session_name += f'NM10-'

        from markov_lm.Model_add import AddModelWithAttentionStacked,AddModelBertInterface
        from markov_lm.Model_add import AddModelWithAttentionMixed,AddModelBertInterface
        # CLS[0] = AddModelWithAttentionStacked
        CLS[0] = AddModelWithAttentionMixed
        ...
        # from markov_lm.Model_add import AddModelBertInsampleMethod-shuffle-NM10-maxPos0--S29-taskduie-ce-shuffle1-AddModelWithAttentionStacked-D4-E100-K50-KE11-IPL100-DenseRelu11-Layernorm1-Dropout0.0-Gradnorm0-loglr-4.0-UseInputImage0-1i29_205_0.08566.pklterface, AddModelBertInterfaceConfig
    ...
    if 0:
        ...
        if conf.task=='refill':
            ...
            conf.loss = lambda item:conf.model.loss(
                ...
                z    = item['masked'],
                mask = item['mask'],
                # summer = torch.gather(item['unmasked'],index=item['masked'],dim=1)!=conf.dataset.char2idx('[sep]'),
                )
            conf.grad_loss = conf.loss
