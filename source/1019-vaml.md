...
$$
\begin{align}
-D_{KL}(q(z|x) || p(z|x,m)) &= \sum_z q_w(z|x) \log {p(z|x,m) \over  q_w(z|x)}
\\
...
$$
\begin{align}
L_2 -L_3 &= c + \sum_{x,z} p_{data}(x)q_e(z|x) \log {q_r(x|z)w(z)
\over p_{data}(x)q_e(z|x) } 
...
-D_{KL}( p(x) q(y|x) || q(x) q(y|x)) &= -D_{KL}(p(x)||q(x))
\end{align}$$
...
&=\left( \begin{aligned}
  ...
  &- D_{KL}(p(x)  e(z|x) ||  w(z)g(x|z) )
    \end{aligned} \right)  
    \\
- D_{KL}(\sum_z p(x)e(z|x)|| \sum_z  w(z)g(x|z)) &\geq
- D_{KL}(p(x)  e(z|x) ||  w(z)g(x|z) )\\
\end{align}
$$
...
-D_{KL}(p_{data}(y,x)||q(y,x)) &\geq c + E_{p_{data}(x)}[ \sum_z q_e(z|x) \left( \log  { q_r(y|z)w(z)  \over \sum_z q_e(z|x) w(z) } \right) ]
\end{align}
$$
